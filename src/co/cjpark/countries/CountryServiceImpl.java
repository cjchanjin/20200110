...
public class CountryServiceImpl extends CountryService {
	...
	private final String SEARCH_SELECT="select * from countries where region_id=?";
	private final String CTR_INSERT="insert into countries values (?,?,?)";
	private final String CTR_UPDATE="update countries set country_name=?, region_id=? where country_id=?";
	private final String CTR_DELETE="delete from countries where country_id=?";
			
	
	@Override
	public List<CountryDto> allSelect() throws SQLException {
	...
	@Override
	public CountryDto select(int key) throws SQLException {
	...
	public int insert(CountryDto dto) throws SQLException {
		psmt=conn.prepareStatement(CTR_INSERT);
	...
	public int update(CountryDto dto) throws SQLException {
		psmt=conn.prepareStatement(CTR_UPDATE);
	...
	public int delete(CountryDto dto) throws SQLException {
		psmt = conn.prepareStatement(CTR_DELETE);
		psmt.setString(1, dto.getCountry_id());
		int n = psmt.executeUpdate();
